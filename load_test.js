...
export default function (data) {
  ...
  // if (res.status !== 200) console.log(`/elders/${ELDER_ID}/home GET Status: ${res.status}, Body: ${res.body.substring(0, 200)}...`);
  // sleep(1);

  // 현재 회원 정보 가져오기
  res = http.get(`${BASE_URL}/member`, { headers: authHeaders });
  check(res, { '회원 정보 가져오기 상태 200 확인': (r) => r.status === 200 });
  if (res.status !== 200) console.log(`/member GET Status: ${res.status}, Body: ${res.body.substring(0, 200)}...`);
  sleep(1);

  // 어르신 목록 가져오기
  res = http.get(`${BASE_URL}/elders`, { headers: authHeaders });
  check(res, { '어르신 목록 가져오기 상태 200 확인': (r) => r.status === 200 });
  if (res.status !== 200) console.log(`/elders GET Status: ${res.status}, Body: ${res.body.substring(0, 200)}...`);
  sleep(1);

  // 어르신 건강정보 조회
  res = http.get(`${BASE_URL}/elders/health-info`, { headers: authHeaders });
  check(res, { '어르신 건강 정보 조회 상태 200 확인': (r) => r.status === 200 });
  if (res.status !== 200) console.log(`/elders/health-info GET Status: ${res.status}, Body: ${res.body.substring(0, 200)}...`);
  sleep(1);
  ...
  // if (res.status !== 201) console.log(`/elders/{elderId}/health-info POST Status: ${res.status}, Body: ${res.body.substring(0, 200)}...`);
  // sleep(1);
  ...
  if (res.status !== 200) console.log(`/elders/{elderId}/blood-sugar/weekly (BEFORE_MEAL) GET Status: ${res.status}, Body: ${res.body.substring(0, 200)}...`);
  sleep(1);
  ...
  if (res.status !== 200) console.log(`/elders/{elderId}/blood-sugar/weekly (AFTER_MEAL) GET Status: ${res.status}, Body: ${res.body.substring(0, 200)}...`);
  sleep(1);
  ...
  // if (res.status !== 200) console.log(`/elders/{elderId}/care-call-setting POST Status: ${res.status}, Body: ${res.body.substring(0, 200)}...`);
  // sleep(1);

  // 어르신 전화 시간대 조회
  res = http.get(`${BASE_URL}/elders/${ELDER_ID}/care-call-setting`, { headers: authHeaders });
  check(res, { '케어콜 설정 조회 상태 200 확인': (r) => r.status === 200 });
  if (res.status !== 200) console.log(`/elders/{elderId}/care-call-setting GET Status: ${res.status}, Body: ${res.body.substring(0, 200)}...`);
  sleep(1);
  ...
  // if (res.status !== 200) console.log(`/elders POST Status: ${res.status}, Body: ${res.body.substring(0, 200)}...`);
  // sleep(1);

  // 어르신 개인정보 수정 (등록된 어르신 ID 사용)
  // if (newElderId) {
  ...
  //   if (res.status !== 200) console.log(`/elders/{newElderId} POST Status: ${res.status}, Body: ${res.body.substring(0, 200)}...`);
  //   sleep(1);
  // }
  ...
  if (res.status !== 200) console.log(`/elders/{elderId}/health-analysis GET Status: ${res.status}, Body: ${res.body.substring(0, 200)}...`);
  sleep(1);
  ...
  if (res.status !== 200) console.log(`/elders/{elderId}/meals GET Status: ${res.status}, Body: ${res.body.substring(0, 200)}...`);
  sleep(1);
  ...
  if (res.status !== 200) console.log(`/elders/{elderId}/medication GET Status: ${res.status}, Body: ${res.body.substring(0, 200)}...`);
  sleep(1);
  ...
  // if (res.status !== 200) console.log(`/member POST Status: ${res.status}, Body: ${res.body.substring(0, 200)}...`);
  // sleep(1);
  ...
  if (res.status !== 200) console.log(`/elders/{elderId}/mental-analysis GET Status: ${res.status}, Body: ${res.body.substring(0, 200)}...`);
  sleep(1);
  ...
  // if (res.status !== 200) console.log(`/payments/reserve POST Status: ${res.status}, Body: ${res.body.substring(0, 200)}...`);
  // sleep(1);

  // 공지사항 목록 조회
  res = http.get(`${BASE_URL}/notices`, { headers: authHeaders });
  check(res, { '공지사항 목록 조회 상태 200 확인': (r) => r.status === 200 });
  if (res.status !== 200) console.log(`/notices GET Status: ${res.status}, Body: ${res.body.substring(0, 200)}...`);
  sleep(1);
  ...
  if (res.status !== 200) console.log(`/elders/{elderId}/sleep GET Status: ${res.status}, Body: ${res.body.substring(0, 200)}...`);
  sleep(1);

  // 회원의 어르신 구독 정보 조회
  res = http.get(`${BASE_URL}/elders/subscriptions`, { headers: authHeaders });
  check(res, { '회원의 어르신 구독 정보 조회 상태 200 확인': (r) => r.status === 200 });
  if (res.status !== 200) console.log(`/elders/subscriptions GET Status: ${res.status}, Body: ${res.body.substring(0, 200)}...`);
  sleep(1);
  ...
  // if (res.status !== 200) console.log(`/elders/{elderId}/weekly-stats GET Status: ${res.status}, Body: ${res.body.substring(0, 200)}...`);
  // sleep(1);
